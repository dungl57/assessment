            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'github', usernameVariable: 'GIT_USER_NAME', passwordVariable: 'GIT_PASSWORD')]) {
                        ...
                        // 4. Check if there were any changes, and handle the exit code correctly.
                        // FIX 2: Combine git diff and conditional logic.
                        if (sh(returnStatus: true, script: "git diff --quiet --exit-code ./k8s-manifest/deployment.yaml") != 0) {
                            // 5. Only commit and push IF there were changes
                                ...
                                sh "git push https://${GIT_USER_NAME}:${GIT_PASSWORD}@github.com/${GIT_USER_NAME}/${GIT_REPO_NAME}.git HEAD:dev"
